/* IRLibTimer.h from IRLib - an Arduino library for infrared encoding and
 * decoding
 ...
 *
 * This file defines which timer you wish to use. Different versions of Arduino
 * and related boards have different timers available to them. For various
 * reasons you might want to use something other than timer 2. Some boards do
 * not have timer 2. This attempts to detect which type of board you are using.
 * You need uncomment wish to use on your board. You probably will not need to
 * include this in your program unless you want to see which timer is being used
 * and which board has been detected. This information came from an alternative
 * fork of the original Ken Shirriff library found here
...
/* In this section reattempt to detect your hardware type and give you the
 ...
 * unless you're using nonstandard hardware that would require you to do so. If
 * you wish to use the bit-bang PWM output will specify that AFTER this section
 * as an override.
 */
// Arduino Mega
 ...
 */
// #define IR_SEND_BIT_BANG 3 // Be sure to set this pin number if un-commented
 ...
 * the following IR_RECV_TIMERx lines below that.
 * NOTE: You are responsible for ensuring that the timer you are specifying is
 * available on your hardware. You should only choose timers which are shown as
 * available for your hardware platform as shown in the defines in the
 * IR_SEND_TIMER section above.
 */
 ...
 */
...
#elif defined(IR_SEND_TIMER4_HS) // Defines for timer4 (10 bits, high speed
                                    // option)
    ...
    #endif
#else // Unknown timer
    #error "Internal code config error, no known IR_SEND_TIMER# defined\n"
#endif

/* This section sets up the 50 microsecond interval timer used by the IRrecv
 * receiver class. The various timers hhave already been selected earlier in
 * this file.
 */
...
#elif defined(IR_RECV_TIMER4_HS) // Defines for timer4 (10 bits, high speed
                                    // option)
    ...
    #define IR_RECV_CONFIG_TICKS() ({\
        ...
        TCNT5 = 0;})
#else // Unknown timer
    #error "Internal code config error, no known IR_RECV_TIMER# defined\n"
#endif
...
/* Leonardo not teensy. When using Timer4 output is on 13. Therefore disabling
 * blink LED. You can add an LED elsewhere if you want
 */
#define BLINKLED 1
